...
#if defined(__x86_64__) || defined(_M_X64) || defined(X86_64)
#define SPNG_X86_64
#endif

#elif defined(__aarch64__) || defined(_M_ARM64) || defined(AARCH64)
...
static void defilter_avg3(size_t rowbytes, unsigned char *row,
                          const unsigned char *prev);
static void defilter_avg4(size_t rowbytes, unsigned char *row,
                          const unsigned char *prev);
static void defilter_paeth3(size_t rowbytes, unsigned char *row,
                            const unsigned char *prev);
static void defilter_paeth4(size_t rowbytes, unsigned char *row,
                            const unsigned char *prev);
...
static void u16_row_to_host(void *row, size_t size) {
    ...
    for (i = 0; i < n; i++) { px[i] = read_u16(&px[i]); }
}
...
static void u16_row_to_bigendian(void *row, size_t size) {
    ...
    for (i = 0; i < n; i++) { write_u16(&px[i], px[i]); }
}

static void rgb8_row_to_rgba8(const unsigned char *row, unsigned char *out,
                              uint32_t n) {
    uint32_t i;
    for (i = 0; i < n; i++) {
...
static unsigned num_channels(const struct spng_ihdr *ihdr) {
    ...
    case SPNG_COLOR_TYPE_INDEXED: return 1;
    default: return 0;
    }
}
...
static int calculate_scanline_width(const struct spng_ihdr *ihdr,
                                    uint32_t width, size_t *scanline_width) {
    if (ihdr == NULL || !width)
...
static int calculate_image_width(const struct spng_ihdr *ihdr, int fmt,
                                 size_t *len) {
    ...
    switch (fmt) {
    case SPNG_FMT_RGBA8:
    case SPNG_FMT_GA16: bytes_per_pixel = 4; break;
    case SPNG_FMT_RGBA16: bytes_per_pixel = 8; break;
    case SPNG_FMT_RGB8: bytes_per_pixel = 3; break;
    case SPNG_FMT_PNG:
    case SPNG_FMT_RAW: {
    ...
    case SPNG_FMT_G8: bytes_per_pixel = 1; break;
    case SPNG_FMT_GA8: bytes_per_pixel = 2; break;
...
static int calculate_image_size(const struct spng_ihdr *ihdr, int fmt,
                                size_t *len) {
    if (ihdr == NULL || len == NULL)
        return SPNG_EINTERNAL;
...
static int write_header(spng_ctx *ctx, const uint8_t chunk_type[4],
                        size_t chunk_length, unsigned char **data) {
...
static int write_chunk(spng_ctx *ctx, const uint8_t type[4], const void *data,
                       size_t length) {
...
#if ZLIB_VERNUM >= 0x1290 && !defined(SPNG_USE_MINIZ) && !defined(SPNG_USE_ZLIB_RS)
...
#pragma message( \
    "inflateValidate() not available, SPNG_CTX_IGNORE_ADLER32 will be ignored")
#endif
...
*/
static int spng__inflate_stream(spng_ctx *ctx, char **out, size_t *len,
                                size_t extra, const void *start_buf,
                                size_t start_len) {
...
static void defilter_up(size_t bytes, unsigned char *row,
                        const unsigned char *prev) {
    size_t i;
    for (i = 0; i < bytes; i++) { row[i] += prev[i]; }
}
...
static int32_t filter_sum(const unsigned char *prev_scanline,
                          const unsigned char *scanline, size_t size,
                          unsigned bytes_per_pixel, const unsigned filter) {
...
*/
static uint16_t sample_to_target(uint16_t sample, unsigned bit_depth,
                                 unsigned sbits, unsigned target) {
    if (bit_depth == sbits) {
        if (target == sbits)
        ...
        sample = sample >>
            (bit_depth - sbits); /* Shift significant bits to bottom */
...
static inline void gamma_correct_row(unsigned char *row, uint32_t pixels,
                                     int fmt, const uint16_t *gamma_lut) {
    uint32_t i;
...
static inline void scale_row(unsigned char *row, uint32_t pixels, int fmt,
    ...
    if (fmt == SPNG_FMT_RGBA8) {
        ...
        for (i = 0; i < pixels; i++) {
            row[i * 2] = sample_to_target(
                row[i * 2], depth, sbit->grayscale_bits, 8);
        }
    }
}

/* Expand to *row using 8-bit palette indices from *scanline */
static void expand_row(unsigned char *row, const unsigned char *scanline,
...
static void unpack_scanline(unsigned char *out, const unsigned char *scanline,
                            uint32_t width, unsigned bit_depth, int fmt) {
...
static int check_ihdr(const struct spng_ihdr *ihdr, uint32_t max_width,
                      uint32_t max_height) {
    ...
    while (!(ret = read_header(ctx))) {
        ...
        if (is_small_chunk(chunk.type)) {
            ...
            ret = read_chunk_bytes(ctx,
                                   chunk.length > 768 ? 768 : chunk.length);
            if (ret)
                return ret;
        }
        ...
        } else /* Arbitrary-length chunk */
        {
            if (!memcmp(chunk.type, type_exif, 4)) {
                ...
                const unsigned char *keyword_nul = memchr(
                    data, 0, chunk.length > 80 ? 80 : chunk.length);
                ...
                text->translated_keyword = text->keyword +
                ...
                if (splt->sample_depth == 16) {
                    for (k = 0; k < splt->n_entries; k++) {
                        ...
                        splt->entries[k].frequency = read_u16(data + k * 10 +
                                                              8);
                    }
                } else if (splt->sample_depth == 8) {
                    for (k = 0; k < splt->n_entries; k++) {
...
int spng_decode_image(spng_ctx *ctx, void *out, size_t len, int fmt,
                      int flags) {
    if (ctx == NULL)
        return 1;
    if (ctx->encode_only)
    ...
    if (!depth_target || depth_target > 16 || !processing_depth ||
        ...
        sb->grayscale_bits > processing_depth || !sb->alpha_bits ||
    ...
    if (f.indexed) {
        ...
        for (i = 0; i < 256; i++) {
            ...
            red = sample_to_target(
                ctx->plte.entries[i].red, 8, sb->red_bits, 8);
            green = sample_to_target(
                ctx->plte.entries[i].green, 8, sb->green_bits, 8);
            blue = sample_to_target(
                ctx->plte.entries[i].blue, 8, sb->blue_bits, 8);
            alpha = sample_to_target(
                ctx->plte.entries[i].alpha, 8, sb->alpha_bits, 8);
        ...
        if (fmt & (SPNG_FMT_RGBA8 | SPNG_FMT_RGBA16)) {
            if (ihdr->color_type == SPNG_COLOR_TYPE_TRUECOLOR) {
                if (ihdr->bit_depth == 16) {
                   ...
                   SPNG_COLOR_TYPE_GRAYSCALE) // fmt == SPNG_FMT_GA8 &&
        {
            if (ihdr->bit_depth == 16) {
    ...
    do {
        ...
        ret = spng_decode_row(
            ctx, (unsigned char *)out + ioffset, ctx->image_width);
    } while (!ret);
...
static int write_idat_bytes(spng_ctx *ctx, const void *scanline, size_t len,
                            int flush) {
...
static int encode_scanline(spng_ctx *ctx, const void *scanline, size_t len) {
    ...
    ret = write_idat_bytes(
        ctx, filtered_scanline - 1, scanline_width, Z_NO_FLUSH);
    if (ret)
        return encode_err(ctx, ret);
...
int spng_encode_image(spng_ctx *ctx, const void *img, size_t len, int fmt,
                      int flags) {
    if (ctx == NULL)
        return 1;
    if (!ctx->state)
...
int spng_get_unknown_chunks(spng_ctx *ctx, struct spng_unknown_chunk *chunks,
                            uint32_t *n_chunks) {
    if (ctx == NULL)
        return 1;
    int ret = read_chunks(ctx, 0);
...
int spng_set_unknown_chunks(spng_ctx *ctx, struct spng_unknown_chunk *chunks,
                            uint32_t n_chunks) {
    if (!n_chunks)
        return 1;
...
static void defilter_avg3(size_t rowbytes, unsigned char *row,
    ...
    while (rb >= 4) {
        ...
        avg = _mm_sub_epi8(
            avg, _mm_and_si128(_mm_xor_si128(a, b), _mm_set1_epi8(1)));
...
static void defilter_sub3(size_t rowbytes, unsigned char *row) {
    ...
    for (; rp < rp_stop;) {
        ...
        vst1_lane_u32(
            png_ptr(uint32_t, rp), png_ldr(uint32x2_t, &vdest.val[0]), 0);
        rp += 3;
        vst1_lane_u32(
            png_ptr(uint32_t, rp), png_ldr(uint32x2_t, &vdest.val[1]), 0);
        rp += 3;
        vst1_lane_u32(
            png_ptr(uint32_t, rp), png_ldr(uint32x2_t, &vdest.val[2]), 0);
        rp += 3;
        vst1_lane_u32(
            png_ptr(uint32_t, rp), png_ldr(uint32x2_t, &vdest.val[3]), 0);
        rp += 3;
    }
}
